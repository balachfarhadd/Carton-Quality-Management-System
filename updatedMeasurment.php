...
if (isset($_GET['id'])) {
    ...
    $sql = "SELECT * FROM `measurmentproduct` WHERE measurment_id = $id";
    ...
    if ($result) {
        ...
        echo '<div class="container my-5">
                 <form action="update.php" method="post">
                    <h1> STOCK UPDATE</h1>
                     <input type="hidden" name="id" value="'.$row['measurment_id'] .'">
                     <div class="form-group">
                         <label for="category">Product Measurment:</label>
                         <input type="text" class="form-control" name="measurment" value="' . $row['measurement_value'] . '">
                     </div>
